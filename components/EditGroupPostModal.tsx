...
interface EditGroupPostModalProps {
  post: GroupPost;
  onClose: () => void;
}

const EditGroupPostModal: React.FC<EditGroupPostModalProps> = ({ post, onClose }) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await supabase.from('group_posts').update({ content }).eq('id', post.id);
    setLoading(false);
    if (!error) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-8 animate-fade-in-up">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Modifier le post</h2>
...
export default EditGroupPostModal;

            ...
            <div className="flex-grow overflow-hidden">
                ...
                <div className="flex justify-between items-center mt-1">
                     <p className={`text-xs truncate font-medium ${unread_count > 0 ? 'text-slate-900 font-bold' : 'text-slate-400'}`}>
                        {isLastMessageFromMe && <span className="mr-1">{last_message?.is_read ? <CheckCheck size={14} className="inline" style={{ color: '#fff000' }} /> : <CheckCheck size={14} className="inline text-slate-300" />}</span>}
